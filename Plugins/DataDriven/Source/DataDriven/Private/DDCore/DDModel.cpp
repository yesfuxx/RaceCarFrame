...
{
	//运行Tick组的Tick函数
	// for (IDDOO* Item : ObjectTickGroup)
	// 	Item->DDTick(DeltaSeconds);
	...
	//循环运行激活生命周期函数
	for (IDDOO* Item : ObjectActiveGroup)
	{
		//如果激活成功
		if (Item->ActiveLife())
			TempObjectGroup.Push(Item);
	}
	//将运行完生命周期的对象移出生命周期组，并且将运行Tick运行的对象添加到Tick组
	for (IDDOO* Item : TempObjectGroup)
	{
		ObjectActiveGroup.Remove(Item);
		if (Item->IsAllowTickEvent)
			ObjectTickGroup.Push(Item);
	}

	//执行释放对象函数，清空释放组
	for (IDDOO* Item : ObjectReleaseGroup)
		Item->DDRelease();
	...
	//运行销毁对象组的销毁生命周期函数
	for (IDDOO* Item : ObjectDestroyGroup)
	{
		//如果销毁生命周期执行完毕
		if (Item->DestroyLife())
		{
			//添加对象到释放对象组
			ObjectReleaseGroup.Push(Item);
			//添加对象到临时组
			TempObjectGroup.Push(Item);
			//清除掉该销毁的对象
			ObjectGroup.Remove(Item->GetObjectName());
			ObjectClassGroup.Find(Item->GetClassName())->Remove(Item);
			//如果类对象数组为空
			if (ObjectClassGroup.Find(Item->GetClassName())->Num() == 0)
				ObjectClassGroup.Remove(Item->GetClassName());
		}
	}

	//把销毁的对象从销毁对象组移除
	for (IDDOO* Item : TempObjectGroup)
		ObjectDestroyGroup.Remove(Item);

	//清空临时对象组
	TempObjectGroup.Empty();
	//处理预销毁对象组，如果对象进入稳定状态，就转到临时对象组
	for (IDDOO* Item : PreObjectDestroyGroup)
	{
		if (Item->RunState == EBaseObjectState::Stable)
		{
			//添加稳定状态的对象到临时对象组
			TempObjectGroup.Push(Item);
		}
	}	
	//从预销毁对象组中清除稳定状态对象，添加到销毁对象组
	for (IDDOO* Item : TempObjectGroup)
	{
		PreObjectDestroyGroup.Remove(Item);
		ObjectDestroyGroup.Push(Item);
		//移除出帧函数组
		ObjectTickGroup.Remove(Item);
	}
}
